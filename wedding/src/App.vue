...
const newItem = ref({ name: '', planned: '', actual: '', paid: false, memo: '', date: '', vendor: '', category: CATEGORIES[0] })
...
onMounted(() => {
  channel = supabase.channel('wedding-realtime')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'wedding_items' }, (p) => {
      ...
      else if (p.eventType === 'UPDATE') { const idx = items.value.findIndex(i => i.id === p.new.id); if (idx >= 0) items.value[idx] = p.new }
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'wedding_checklist' }, (p) => {
      ...
      else if (p.eventType === 'UPDATE') { const idx = checklist.value.findIndex(c => c.id === p.new.id); if (idx >= 0) checklist.value[idx] = p.new }
  ...
  <div class="app">
    ...
    <template v-else>
      ...
      <template v-else>
            ...
            <div v-if="view === 'home'">
              ...
              <div v-if="showSettings" class="settings-panel">
                ...
                <div class="setting-row">
                  ...
                  <div class="setting-input-wrap">
                    <input v-model.number="budget" type="number" class="setting-input" @change="saveSettings" />
              ...
              <div class="budget-card" :class="{ over: overBudget }">
                <div class="budget-header">
                  ...
                  <div class="budget-badge" :class="overBudget ? 'badge-over' : 'badge-ok'">
                    {{ overBudget ? '⚠️ 초과' : '✅ 여유' }}
                  </div>
                </div>
                ...
                <div class="budget-stats">
                  <div class="stat-item">
                    ...
                    <div class="stat-value" :class="{ red: overBudget }">{{ fmt(totalActual) }}원</div>
                  </div>
                  <div class="stat-divider"></div>
                  <div class="stat-item">
                    <div class="stat-label">계획</div>
                    <div class="stat-value">{{ fmt(totalPlanned) }}원</div>
                  </div>
                  <div class="stat-divider"></div>
                  <div class="stat-item">
                    <div class="stat-label">잔액</div>
                    <div class="stat-value" :class="overBudget ? 'red' : 'green'">{{ fmt(balance) }}원</div>
                  </div>
                </div>
              </div>
              ...
              <div class="cat-list">
                <div v-for="(cat, i) in CATEGORIES" :key="cat"
                  ...
                  <div class="cat-left">
                    <span class="cat-emoji">{{ CAT_ICONS[i] }}</span>
                    <div>
                      <div class="cat-name">{{ cat }}</div>
                      <div class="cat-sub">{{ catItems(cat).length }}개 항목</div>
                    </div>
                  </div>
                  <div class="cat-right">
                    <div v-if="catOver(cat)" class="over-badge">초과!</div>
                    <div class="cat-actual" :class="{ red: catOver(cat) }">{{ fmt(catActual(cat)) }}원</div>
                    <div class="cat-planned">/ {{ fmt(catPlanned(cat)) }}원 계획</div>
                    <div v-if="catBudgets[cat] > 0" class="mini-bar">
                      <div class="mini-fill" :style="{ width: catProgress(cat) + '%', background: catOver(cat) ? '#ff4757' : CAT_COLORS[i] }"></div>
                    </div>
                  </div>
        ...
        <!-- FAB -->
        <button v-if="tab === 'home' && view === 'home'" class="fab" @click="showQuickAdd = true">+</button>
          ...
          <div class="modal">
            ...
            <div class="cat-chips">
              <button v-for="(cat, i) in CATEGORIES" :key="cat"
                class="cat-chip" :class="{ active: newItem.category === cat }"
                :style="newItem.category === cat ? { background: CAT_COLORS[i], color: 'white', borderColor: CAT_COLORS[i] } : {}"
                @click="newItem.category = cat">{{ CAT_ICONS[i] }} {{ cat }}</button>
            </div>
            <input v-model="newItem.name" placeholder="항목명 *" class="input-field" />
            <input v-model="newItem.vendor" placeholder="업체명" class="input-field" />
            <div class="input-row">
...
.app { max-width: 480px; margin: 0 auto; background: #f7f3f3; min-height: 100vh; min-height: 100dvh; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; position: relative; }
...
.nav-btn { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 3px; padding: 10px 0; background: none; border: none; cursor: pointer; color: #ccc; transition: color 0.15s; }
.nav-btn.active { color: #ff6b6b; }
.nav-icon { font-size: 22px; }
.nav-label { font-size: 11px; font-weight: 600; }

/* FAB */
.fab { position: fixed; bottom: calc(74px + env(safe-area-inset-bottom, 0px)); right: max(16px, calc(50% - 224px)); width: 54px; height: 54px; border-radius: 50%; background: #ff6b6b; color: white; font-size: 30px; border: none; cursor: pointer; box-shadow: 0 4px 18px rgba(255,107,107,0.45); display: flex; align-items: center; justify-content: center; z-index: 99; line-height: 1; }
...
.paid-tag { background: #d3f9d8; color: #2f9e44; font-size: 11px; padding: 2px 8px; border-radius: 8px; white-space: nowrap; }
...
.btn-check-add { background: #ff6b6b; color: white; border: none; border-radius: 12px; padding: 13px 18px; font-size: 15px; font-weight: 700; cursor: pointer; white-space: nowrap; }
...
.btn-check-del { background: none; border: none; color: #ddd; font-size: 15px; cursor: pointer; padding: 4px 6px; min-width: 28px; }
...
.cat-chip { padding: 8px 14px; border: 1px solid #eee; border-radius: 20px; font-size: 13px; cursor: pointer; background: white; transition: 0.15s; white-space: nowrap; }
